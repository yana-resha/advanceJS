...
function createValidationForm (container) {
  ...
  // номер карты
  const numberCardInput = el("input.form-control", { type: "text", oninput() {
    setChildren(errSpan, '')
  ...
  const dateInput = el("input.form-control", { type: "text", oninput() {
    setChildren(errSpan, '')
  ...
  const cvcInput = el("input.form-control", { type: "text", oninput() {
  ...
  const emailInput = el("input.form-control", { type: "email", oninput() {
    setChildren(errSpan, '')
  ...
  const btnSubmitForm = el("button.btn.btn-primary.mt-5", {type: "submit", onclick(e) {
    e.preventDefault()}
  } ,"Отправить");
